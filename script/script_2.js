javascript: (function () {
  ...
  document.addEventListener("mouseenter", toggleHighlightElement);
  document.addEventListener("mouseleave", toggleHighlightElement);
  ...
  function toggleHighlightElement(
    e,
    propertiesToChange = {
      boxShadow: "0px 0px 0px 3px red !important",
      cursor: "pointer !important",
    }
  ) {
    const { el } = (highlightedElement.el = e.target);
  ...
  function setItemCssProperties(el, props) {
    ...
    propKeys.forEach((prop) => (el[prop] = propKeys[prop]));
  }
